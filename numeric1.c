...
void false_position( double x_l, double x_u ) {
    ...
    double x_r = x_u - (function(x_u) * (x_u - x_l))/(function(x_u) - function(x_l)) ;

    printf( "\t %d\t %f\t %f\t %f\t %f\t \n", no, x_l, x_u, x_r, function(x_r));
    no++;
    ...
    while ( E_a > 0.000001 ) {
        double x_rnew = x_u - (function(x_u) * (x_u - x_l))/(function(x_u) - function(x_l)) ;

        E_a = fabs( (x_rnew - x_r) / x_rnew ) ;
